...
behavior island_model_worker(
  ...
  return {
    [self](init_population) {
      ...
      aout(self) << "end of init " << state.current_island << " : " << state.population << std::endl;
    },
    [self](execute_generation) {
      ...
      aout(self) << "end of exectution " << state.current_island << " : " << state.population << std::endl;
    },
    ...
    [self](finish) {
      ...
      aout(self) << "end of finish " << state.current_island << std::endl;
      self->quit();
    }
  };
}
...
behavior island_model_dispatcher(
  ...
  for (island_id i = 0; i < islands; ++i) {
    ...
    system_message(self, "Spawning new island worker with id: ", i, " (actor id: ", island.id(), ")");
...
class island_model_driver : private base_driver {
 public:
  ...
  void run() {
    ...
    std::cout << "requested shutfown" << std::endl;
  }
};
